...
mod hypertext_tests {
    ...
    fn test_hypertext_svg_content_is_valid() {
        let icons = vec![
            ("home_outline", Icon { name: IconName::Home, variant: Some(Variant::Outline), }),
            ("check_circle_solid", Icon { name: IconName::CheckCircle, variant: Some(Variant::Solid), }),
            ("envelope_mini", Icon { name: IconName::Envelope, variant: Some(Variant::Mini), }),
            ("shopping_cart_micro", Icon { name: IconName::ShoppingCart, variant: Some(Variant::Micro), }),
        ];

        for (name, icon) in icons {
            let svg_content = icon.render().into_inner();
            assert!(svg_content.starts_with("<svg"), "{} should start with <svg", name);
            assert!(svg_content.ends_with("</svg>"), "{} should end with </svg>", name);
    ...
    fn test_all_icon_variants_available() {
        ...
        let _home_outline = Icon { name: IconName::Home, variant: Some(Variant::Outline), };
        let _home_solid = Icon { name: IconName::Home, variant: Some(Variant::Solid), };
        let _home_mini = Icon { name: IconName::Home, variant: Some(Variant::Mini), };
        let _home_micro = Icon { name: IconName::Home, variant: Some(Variant::Micro), };

        let _user_plus_outline = Icon { name: IconName::UserPlus, variant: Some(Variant::Outline), };
        let _user_plus_solid = Icon { name: IconName::UserPlus, variant: Some(Variant::Solid), };
        let _user_plus_mini = Icon { name: IconName::UserPlus, variant: Some(Variant::Mini), };
        let _user_plus_micro = Icon { name: IconName::UserPlus, variant: Some(Variant::Micro), };

        let _envelope_outline = Icon { name: IconName::Envelope, variant: Some(Variant::Outline), };
        let _envelope_solid = Icon { name: IconName::Envelope, variant: Some(Variant::Solid), };
        let _envelope_mini = Icon { name: IconName::Envelope, variant: Some(Variant::Mini), };
        let _envelope_micro = Icon { name: IconName::Envelope, variant: Some(Variant::Micro), };
    }

    #[test]
    fn test_icon_content_different_between_variants() {
        let home_outline = Icon { name: IconName::Home, variant: Some(Variant::Outline), };
        let home_solid = Icon { name: IconName::Home, variant: Some(Variant::Solid), };
        let home_mini = Icon { name: IconName::Home, variant: Some(Variant::Mini), };
        let home_micro = Icon { name: IconName::Home, variant: Some(Variant::Micro), };
        
        let envelope_outline = Icon { name: IconName::Envelope, variant: Some(Variant::Outline), };
        let user_plus_solid = Icon { name: IconName::UserPlus, variant: Some(Variant::Solid), };
        let shopping_cart_solid = Icon { name: IconName::ShoppingCart, variant: Some(Variant::Solid), };
        
        // Ensure different variants have different content
        assert_ne!(home_outline.render().into_inner(), home_solid.render().into_inner());
        assert_ne!(home_solid.render().into_inner(), home_mini.render().into_inner());
        assert_ne!(home_mini.render().into_inner(), home_micro.render().into_inner());

        // Ensure different icons have different content
        assert_ne!(home_outline.render().into_inner(), envelope_outline.render().into_inner());
        assert_ne!(user_plus_solid.render().into_inner(), shopping_cart_solid.render().into_inner());
    }
    ...
    fn test_icon_sizes_have_correct_viewbox() {
        ...
        assert!(home_outline.render().into_inner().contains("viewBox=\"0 0 24 24\""));
        assert!(home_solid.render().into_inner().contains("viewBox=\"0 0 24 24\""));

        // 20px icons (mini)
        assert!(home_mini.render().into_inner().contains("viewBox=\"0 0 20 20\""));

        // 16px icons (micro)
        assert!(home_micro.render().into_inner().contains("viewBox=\"0 0 16 16\""));
    }
